...
function saveTicket(array $d, ?int $id=null): int {
    $db=getDB();
    $f=['equipo_id','usuario_id','tipo_servicio','prioridad','estado','descripcion','tecnico','fecha_cierre'];
    $v=array_map(fn($k)=>($d[$k]??null)?trim($d[$k]):null,$f);
    if($id){ $set=implode(',',array_map(fn($k)=>"$k=?",$f)); $db->prepare("UPDATE tickets SET $set WHERE id=?")->execute([...$v,$id]); return $id; }
